        ...
        else
        {
            ...
            if([[PosManager getInstance].pos hasHeadset]){  //如果插入刷卡头 也会启动蓝牙 为避免
                ...
                if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"uuidName"] length]>0) {
                    ...
                    self.hub = [MBProgressHUD showHUDAddedTo:self.view animated:YES WithString:[NSString stringWithFormat:@"%@ 搜索中..",s]]; [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(closeHUD:) name:@"closeHUD" object:nil];
