...
- (instancetype)init{
    ...
    if (self) {
        ...
        self.telephone.text = [NSString stringWithFormat:@"021-31125901",L(@"ContactCompany"),[[delegate getConfigDic] objectForKey:@"customerService"]];
        NSString *url;
        if ([[[delegate getConfigDic] objectForKey:@"website"]hasPrefix:@"http://www.jiefengpay.com"]) {
            url = [NSString stringWithFormat:@"%@:\n%@",L(@"CompanyWebsite"),[[delegate getConfigDic] objectForKey:@"website"]];
        }else{
...
- (IBAction)gotoWebview:(UIButton *)sender {
    ...
    [self.superCtrl dismissViewControllerAnimated:NO completion:^{
        ...
        if ([[[delegate getConfigDic] objectForKey:@"website"]hasPrefix:@"http"]) {
            ...
            url = [NSString stringWithFormat:@"http://www.jiefengpay.com",[[delegate getConfigDic] objectForKey:@"website"]];
        }
...
//    NSString *telUrl = [NSString stringWithFormat:@"tel://%@",[[delegate getConfigDic] objectForKey:@"customerService"]] ;
    NSString *telUrl = [NSString stringWithFormat:@"tel://%@31125901",[[delegate getConfigDic] objectForKey:@"customerService"]] ;
