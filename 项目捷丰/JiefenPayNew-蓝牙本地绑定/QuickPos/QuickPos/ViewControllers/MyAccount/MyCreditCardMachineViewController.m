...
-(void)addNumber{
    ...
    [gotoPageController addAction:[PSTAlertAction actionWithTitle:@"绑定" style:PSTAlertActionStyleCancel handler:^(PSTAlertAction *action) {
        NSString*uuidName = [NSString stringWithFormat:@"YL%@",action.alertController.textField.text ];
        if ([uuidName length]>0){
            [_blueToothNumberSaveArray addObject:uuidName];
            NSLog(@"==%@",[_blueToothNumberSaveArray lastObject]);
            [[NSUserDefaults standardUserDefaults] setObject:uuidName forKey:@"uuidName"];
            dispatch_async(dispatch_get_main_queue(), ^{
