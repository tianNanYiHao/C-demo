...
-(void)addNumber{
    ...
    [gotoPageController addAction:[PSTAlertAction actionWithTitle:@"绑定" style:PSTAlertActionStyleCancel handler:^(PSTAlertAction *action) {
        ...
        else if ([action.alertController.textField.text length]  != 10){
            [MBProgressHUD showHUDAddedTo:self.view WithString:@"绑定码输入长度有误"];
        }
        else{
...
- (void)responseWithDict:(NSDictionary *)dict requestType:(NSInteger)type{
    ...
    if (type == REQUEST_GETBULETOOHTHNUMBER) {
        ...
        if ([[[dict objectForKey:@"data"] objectForKey:@"optType"] isEqualToString:@"2"]) { //绑定
            ...
            else if([[[[dict objectForKey:@"data"] objectForKey:@"result"] objectForKey:@"resultCode"]isEqualToString:@"0001"]){
                [MBProgressHUD showHUDAddedTo:self.view WithString:[[[dict objectForKey:@"data"] objectForKey:@"result"] objectForKey:@"message"]];
            }
            else{ //绑定成功
                [_myCreditCardMachineTableView reloadData];
                
                [[NSUserDefaults standardUserDefaults] setObject:[_blueToothNumberSaveArray.lastObject deviceId] forKey:@"SerialNumber"];
            }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [[NSUserDefaults standardUserDefaults] setObject:[_blueToothNumberSaveArray[indexPath.row] deviceId] forKey:@"SerialNumber"];
