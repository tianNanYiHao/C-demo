...
//    
    ...
    NSMutableArray *segmentedArr = [[NSMutableArray alloc]initWithObjects:@"全部",@"收入",@"支出",@"理财",nil];
    
    //初始化UISegmentedControl
    
    UISegmentedControl *segmentedVc = [[UISegmentedControl alloc]initWithItems:segmentedArr];
    
    segmentedVc.frame = CGRectMake(5.0, 5.0, CGRectGetWidth(self.view.bounds)-10, 30.0);
    
    segmentedVc.selectedSegmentIndex = 1;//设置默认选择项索引
    
    segmentedVc.tintColor = [Common hexStringToColor:@"#4c5864"];
    
    segmentedVc.segmentedControlStyle = UISegmentedControlStylePlain;//设置样式
    
    [self.view addSubview:segmentedVc];
    
    [segmentedVc addTarget:self action:@selector(segmentACT:) forControlEvents:UIControlEventValueChanged];
