...
@property (weak, nonatomic) IBOutlet UIView *headImageViewBG;//头背景图
...
//    self.myAccountTableView.tableFooterView = self.OutFootView;
    ...
    [self.tuichu setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    ...
    [rightBtn setBackgroundImage:[UIImage imageNamed:@"serve_more"] forState:UIControlStateNormal];
    [rightBtn addTarget:self action:@selector(showSetup:) forControlEvents:UIControlEventTouchUpInside];
    rightBtn.frame = CGRectMake(0, 0, 20, 20);
    rightBtn.layer.cornerRadius = 17.5;
    UIBarButtonItem *rightItem = [[UIBarButtonItem alloc] initWithCustomView:rightBtn];
    self.navigationItem.leftBarButtonItem = rightItem;
    
       [self createHeadImageVIewBG];
    
}
-(void)createHeadImageVIewBG{
    
    _moneyLabel.text = availableAmtStr;
    _withdrawalLabel.text = cashAvailableAmtStr;
    
    _headicon.image = [UIImage imageNamed:@"wode_logo"];
    _headicon.hidden = YES;
    
    [_headiconView addSubview: self.headImageView];
    
    float w = _headiconView.frame.size.width;
    float h = _headiconView.frame.size.height;
    _headImageView.frame = CGRectMake(0, 0, w, h);
    self.headImageViewBG.backgroundColor = [UIColor colorWithRed:39/255.0 green:44/255.0 blue:54/255.0 alpha:1];
    [_headImageViewBG addSubview:_moneyLabel];
...
- (void)responseWithDict:(NSDictionary *)dict requestType:(NSInteger)type{
    ...
    if(type == REQUSET_USERINFOQUERY && [dict[@"respCode"] isEqual:@"0000"]){
        ...
        picStr = info.pic;
        ID = info.certPid;
        _usernameLabel.text = realNameStr;
        //GCD操作
    ...
    if (type == REQUEST_GETUPHEADIMAGEHEAD) {
        ...
        if ([pic isKindOfClass:[NSNull class]]) {
            ...
            [_myAccountTableView reloadData];
        }
    }
...
//        headerCell.headicon.image = [UIImage imageNamed:@"wode_logo"];
//        headerCell.headicon.hidden = YES;
//        
//        
//        [headerCell.headiconView addSubview: self.headImageView];
//
//        float w = headerCell.headiconView.frame.size.width;
//        float h = headerCell.headiconView.frame.size.height;
//        _headImageView.frame = CGRectMake(0, 0, w, h);
////        headerCell.headicon.layer.cornerRadius = 35;
//        headerCell.contentView.backgroundColor = [UIColor colorWithRed:39/255.0 green:44/255.0 blue:54/255.0 alpha:1];
//        return headerCell;
//    }
//    
    //列表cell绑定
...
//                cell3.titleLabel2.text = [NSString stringWithFormat:@"可提现:%@元",cashAvailableAmtStr];
    ...
    if (indexPath.section == 5) {
        cell.titleLabel.text = [NSString stringWithFormat:@"退出登录"];
        cell.logoImageView.image = nil;
    }
    
    if ((indexPath.section == 0 && indexPath.row == 0 )||indexPath.section == 6) {
    ...
    if (indexPath.section == 5) {
        ...
        cell.titleLabel.frame = CGRectMake(46, 16, 233, 15);
    }
...
//        withdrawalVC.name = info.realName;
        withdrawalVC.name = realNameStr;
        NSLog(@"%@  %@", withdrawalVC.name,realNameStr);
...
- (UIImageView*)makeHeadImageView
{
    ...
    UIImageView * viewMask = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"sidebar_icon_OKOK"]];
