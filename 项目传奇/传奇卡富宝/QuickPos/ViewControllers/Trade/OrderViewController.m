...
@interface OrderViewController ()<ResponseData,BankCarCheckDelegate,PosResponse,UIAlertViewDelegate,HandSignActionDelegate,CSwiperStateChangedListener,DCBlueToothManagerDelegate,UITextFieldDelegate>{
        ...
        if ([_payWay isEqualToString:@"音频"]) {
            ...
            if([[PosManager getInstance].pos hasHeadset]){  //如果插入刷卡头 也会启动蓝牙 为避免
                ...
                if ([[UUIDNAME substringToIndex:2] isEqualToString:@"CQ"]) {
                    ...
                    _dcBlueToothManager.actionType = 1; //刷卡
...
- (void)responseWithDict:(NSDictionary *)dict requestType:(NSInteger)type{
    ...
    if ([[dict objectForKey:@"respCode"]isEqualToString:@"0000"]) {
        ...
        else if(type == REQUEST_CARDSTATU)
        {
            if (_blutType == ItronBlueT){
                NSString *statu =  dict[@"data"][@"result"][@"resultCode"];
                //1002 已认证  1003未认证
                if ([statu isEqualToString:@"1002"]) {
                    [self alertViewPassword];
                }
                if ([statu isEqualToString:@"1003"]) {
                    UIStoryboard *storyboard = [UIStoryboard storyboardWithName:@"Main" bundle:nil];
                    BankCarCheckViewController *BankCarCheckViewController = [storyboard instantiateViewControllerWithIdentifier:@"BankCarCheckViewController"];
                    BankCarCheckViewController.delegate = self;
                    BankCarCheckViewController.carNum = self.cardInfoModel.cardNum;
                    [self.navigationController pushViewController:BankCarCheckViewController animated:YES];
                }
            }
            else if (_blutType == DCBlueT){
                NSString *statu =  dict[@"data"][@"result"][@"resultCode"];
                //1002 已认证  1003未认证
                if ([statu isEqualToString:@"1002"]) {
                    [self dcSwiperWithCardInfo:self.cardInfoModel];
                }
                if ([statu isEqualToString:@"1003"]) {
                    UIStoryboard *storyboard = [UIStoryboard storyboardWithName:@"Main" bundle:nil];
                    BankCarCheckViewController *BankCarCheckViewController = [storyboard instantiateViewControllerWithIdentifier:@"BankCarCheckViewController"];
...
- (void)handSign{
    ...
    handSignViewController.delegate = self;
...
{
    [[NSNotificationCenter defaultCenter] postNotificationName:@"closeBluetooth" object:nil];
    self.cardInfoModel = cardInfo;
    cardInfoModel = cardInfo; //itron 和 动联 用这个model
    
    if ([cardInfo.cardNum length] == 0) {
        [MBProgressHUD showHUDAddedTo:self.view WithString:@"卡号没有获取的，请重新刷"];
        [self.navigationController popViewControllerAnimated:YES];
        return;
    }
    
    //查询卡的认证状态
    [_request searchAlreadyCarNumStatu:cardInfo.cardNum];

//    [self alertViewPassword];
    
}
#pragma mark carCheckDelegate
...
-(void)dcSwiperWithCardInfo:(CardInfoModel*)cardInfo
{
    ...
    if (cardInfo) {
        ...
        [psta addAction:[PSTAlertAction actionWithTitle:L(@"Confirm") handler:^(PSTAlertAction * _Nonnull action) {
            ...
            action.alertController.textField.tag = 10098;
            action.alertController.textField.delegate = self;
...
- (void)alertViewPassword{
    if ([self.navigationController.viewControllers containsObject:self]) {
        if ([[PosManager getInstance].pos hasHeadset]) {
            
            if (self.cardInfoModel) {
                NSLog(@"卡信息 ----> %@",self.cardInfoModel.cardInfo);
                cardInfoModel = self.cardInfoModel;
                dispatch_async(dispatch_get_main_queue(), ^{
                    //回到主线程队列,进行UI的刷新
                    swiperingView.hidden = YES;
                    if (self.cardInfoModel.hasPassword) {
...
- (void) handsignFinished: (UIImage *) img {
    ...
    NSString *longitude = [NSString stringWithFormat:@"%.4f",[AppDelegate getUserBaseData].lon];
    NSString *latitude = [NSString stringWithFormat:@"%.4f",[AppDelegate getUserBaseData].lat];
    //执行图片上传
    Request *req = [[Request alloc]initWithDelegate:self];
