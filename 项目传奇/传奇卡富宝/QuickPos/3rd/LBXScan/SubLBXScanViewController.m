...
@interface SubLBXScanViewController ()<ResponseData>
{
Request *requst;
}
...
- (void)drawBottomItems
{
    ...
    _btnFlash.center = CGPointMake(CGRectGetWidth(_bottomItemsView.frame)/2+70, CGRectGetHeight(_bottomItemsView.frame)/2);
...
-(void)responseWithDict:(NSDictionary *)dict requestType:(NSInteger)type{
    if (type == REQUSET_CodePay) {

            JingDongResultViewController *vc = [JingDongResultViewController new];
            vc.hidesBottomBarWhenPushed = YES;
            vc.transLogNo = dict[@"orderId"];
            vc.stateinfo = dict[@"respDesc"];
            vc.money = self.orderData.orderAmt;
            vc.transDate = dict[@"transDate"];
            vc.transTime = dict[@"transTime"];
