    ...
    if ([identifier isEqualToString:@"PhoneRechargeSegue"]) {
        if([Common isPhoneNumber:self.phoneNumberTextField.text]){
            ...
            if(payType == AccountPayType){
                ...
                //快捷支付
                NSString *str = [self.payLabel.text stringByReplacingOccurrencesOfString:@"￥" withString:@""];
                NSString *orderAmt = [Common orderAmtFormat:str];                [request applyOrderMobileNo:[AppDelegate getUserBaseData].mobileNo MerchanId:@"0001000001" productId:@"0000000000" orderAmt:orderAmt orderDesc:self.phoneNumberTextField.text orderRemark:@"" commodityIDs:@"" payTool:@"03"];
            }
