...
//    [self showAlertView:@"设备连接成功" messgae:peripheral.name];
    ...
    [_dcSwiper cancelTrade];//取消刷卡 (先复位 再刷卡)
...
- (void)onDCEMVCardData:(NSDictionary *)cardDic
{
    ...
    //ic卡Track等效信息
    NSString *enctrack2  = _dcBTDataModel.encTrack2;
    ...
    if ([cardType isEqualToString:@"0"]) {
        ...
        randomNumber = [data55 substringFromIndex:data55.length-8];  //ic卡 ic卡信息加密随机数 需要从data55后面截取后8位
        NSLog(@"9999999999999999  %@",randomNumber);
...
//        NSString *strTrack3Length = [self intToHexString:track3Length lenght:2];
        ...
        NSString *track2 = [encTrack substringToIndex:track2Length*2];
        NSString *track3 = [encTrack substringFromIndex:track2Length*2];
        ...
        NSString *trankLenght = [self intToHexString:[encTrack length]/2 lenght:2];
    ...
    else  if (enctrack2 && enctrack2.length > 0) {
        ...
        NSString *track2 = [enctrack2 substringToIndex:track2Length*2];
        NSString *track3 = [enctrack2 substringFromIndex:track2Length*2];
        
        NSLog(@"KSN信息 ----- > %@",ksn);
        NSLog(@"磁道信息 ----- > %@",enctrack2);
        NSLog(@"二磁数据 ----- > %@",track2);
        NSLog(@"三磁数据 ----- > %@",track3);
        NSLog(@"2磁长度 ----- >%i,16:%@",track2Length,strTrack2Length);
        NSLog(@"3磁长度 ----- >%i,16:%@",track3Length,strTrack3Length);
        ...
        [CardInfo setRandom:[NSString stringWithFormat:@"%@",randomNumber]];
        [CardInfo setCardNO:[NSString stringWithFormat:@"%@",maskedPAN]];
        [CardInfo setExpiryDate:[NSString stringWithFormat:@"%@",_dcBTDataModel.expiryDate]];  //IC卡有效期传4位数到 不需要(+3)特殊
        [CardInfo setMac:[NSString stringWithFormat:@"%@",mac]];
        [CardInfo setSequensNo:serialNum];
        [CardInfo setData55:data55];
        [CardInfo setHasPassword:YES];
        
        NSString *trankLenght = [self intToHexString:[enctrack2 length]/2 lenght:2];
