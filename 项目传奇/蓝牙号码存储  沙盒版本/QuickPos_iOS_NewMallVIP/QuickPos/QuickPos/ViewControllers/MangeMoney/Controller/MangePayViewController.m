...
- (void)viewDidLoad {
    ...
    _amt = [NSNumber numberWithInt:[_amt intValue]*100];  //传入的金额 统一按分处理 
...
- (IBAction)pushToOrder:(id)sender {
        ...
        NSLog(@"_productID%@_number%@_number%@payTool%@_hongBaoID%@_num%@",_productID,_number,_amt,payTool,_hongBaoID,_num);

    if (_hongBaoID == nil) { //如果红包ID不存在 红包个数_num == 0
        [request getManageProductOrderWithProductID:_productID Number:_number amt:_amt tranType:payTool hongBaoID:@"0000" hongbaoNum:_num];
        [MBProgressHUD showHUDAddedTo:self.view animated:YES WithString:@"订单生成中...."];
        NSLog(@"%@======%d",payTool,payType);
    }else
    {
        [request getManageProductOrderWithProductID:_productID Number:_number amt:_amt tranType:payTool hongBaoID:_hongBaoID hongbaoNum:_num];
        [MBProgressHUD showHUDAddedTo:self.view animated:YES WithString:@"订单生成中...."];
        NSLog(@"%@======%d",payTool,payType);
...
-(void)responseWithDict:(NSDictionary *)dict requestType:(NSInteger)type
{
    ...
    if (type == REQUSET_getManageProductOrder) {
        if ([[dict objectForKey:@"msgcode"] isEqualToString:@"02"]) {
            ...
            orderData.orderAmt = [NSString stringWithFormat:@"%d",[[dict objectForKey:@"amount"] intValue]];
