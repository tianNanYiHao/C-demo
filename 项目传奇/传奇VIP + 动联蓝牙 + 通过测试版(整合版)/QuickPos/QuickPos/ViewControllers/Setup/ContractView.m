...
//        self.telephone.text = [NSString stringWithFormat:@"%@:%@",L(@"ContactCompany"),[[delegate getConfigDic] objectForKey:@"customerService"]];
        NSString *url;
        if ([[[delegate getConfigDic] objectForKey:@"website"]hasPrefix:@"http"]) {
            ...
            url = [NSString stringWithFormat:@"%@:\n http://%@",L(@"CompanyWebsite"),[[delegate getConfigDic] objectForKey:@"website"]];
        }
...
- (IBAction)telephone:(UIButton *)sender {
    AppDelegate *delegate = [UIApplication sharedApplication].delegate;
    NSString *telUrl = [NSString stringWithFormat:@"tel://4000013666"] ;
