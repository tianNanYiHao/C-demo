...
- (void)viewDidLoad {
    ...
    if(self.orderData.orderPayType == CardPayType){
        ...
        self.orderAccount.text = self.orderData.orderDesc;
    ...
    if (alertView.tag == CardPayType) {
        ...
        if (buttonIndex == 1) {
            if ([(UITextField*)[alertView textFieldAtIndex:0] text].length == 0) {
                ...
                hud = [MBProgressHUD showMessag:L(@"IsTrading") toView:[[QuickPosTabBarController getQuickPosTabBarController] view]];
                [req acctPay:[AppDelegate getUserBaseData].mobileNo encodetype:nil password:[(UITextField*)[alertView textFieldAtIndex:0] text] mobileMac:self.codeTextField.text acctType:@"00" merchantId:self.orderData.merchantId productId:self.orderData.productId orderId:self.orderData.orderId orderAmt:self.orderData.orderAmt encodeType:nil payType:@"02"];
            }
