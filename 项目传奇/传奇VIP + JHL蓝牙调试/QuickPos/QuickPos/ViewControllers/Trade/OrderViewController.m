...
@property (nonatomic,strong) JHLBTPosVManager   *manager;
...
//                NSLog(@"-=-=-=-=-=orderId...%@-=-=-=-=-=-transLogo...%@=-=-=-=-=-=-=cash...%@=-=-=-=-=-=-",self.orderData.orderId,self.orderData.orderTranslogNo,self.orderData.orderAmt);
        ...
        else if ([_payWay isEqualToString:@"JHL蓝牙"]){
            if([[PosManager getInstance].pos hasHeadset]){  //如果插入刷卡头 也会启动蓝牙 为避免
                [Common showMsgBox:@"您选择了蓝牙刷卡方式" msg:@"检测到您插入了音频,请拔出音频重试" parentCtrl:self];
            }else{
...
-(void)dcBlueToothManagerResponseByCardInfo:(CardInfoModel *)cardInfo{
    ...
    if (cardInfo) {
        ...
        [psta addAction:[PSTAlertAction actionWithTitle:L(@"Confirm") handler:^(PSTAlertAction * _Nonnull action) {
            ...
            {
                NSLog(@"%@-%@-%@-%@-%@-%@-%@-%@-%@",cardInfoModel.cardInfo,password,cardInfoModel.data55,cardInfoModel.sequensNo,cardInfoModel.expiryDate
                      ,self.orderData.merchantId,self.orderData.productId,self.orderData.orderId,self.orderData.orderAmt);
                
                
                [req cardPay:cardInfoModel.cardInfo
                cardPassWord:password
                  iccardInfo:cardInfoModel.data55
                ICCardSerial:cardInfoModel.sequensNo
             ICCardValidDate:cardInfoModel.expiryDate
